...
editor: Andrew Bartee 02/15/2024
  markdown: 
    wrap: 72
---
...
The focus of this overview is to get you used to tools we will be using in class. Before completing it you should have a basic understanding of using R. We will do an introduction in class [(download help file)](../Primer_material/R_primer.R). You should also be comfortable using Rstudio and github [(see help file)](../Primer_material/Tools_overview.qmd).
...
The : means give every integer between the lowest and highest values given.
...
length(x) returns the number of items in the set.

```{r}
max(x)
```

max(x) returns the maximum value in the set of integers.

```{r}
x[x<10]
```

x\[x\<10\] returns all integers in the set that are less than 10 (the value provided).

```{r}
x^2
```

x\^2 returns the square value of each integer in the set.

```{r}
x[ x < 12 & x > 7]
```

x\[x\<12 & x\>7\] returns all values greater than 7 and less than 12.
...
-1:2 and (-1):2 return the same results. They give all integers between the lowest and highest values.

-(1:2) returns the negative value of the integers inside the parenthesis.
...
dim(sleep) returns rows and columns, in that order? but looking at the data returned it appears to be the opposite. This data set has 62 objects of 12 variables.

str returns the size of the dataset. Can I get this value to appear in the environment tab? I do not understand the use of \$ to select columns.
...
With outliers excluded the linear model fits the data better. If the outliers had been included the line of best fit would be much flatter. According to the data, total sleep decreases as brain weight increases.

### EXTRA QUESTIONS

*not required*

![alt text](https://upload.wikimedia.org/wikipedia/commons/9/94/Puffin_Mrkoww.jpg) **Dow Puffin** *Matthew Zalewski / CC BY (<https://creativecommons.org/licenses/by/3.0>)*
